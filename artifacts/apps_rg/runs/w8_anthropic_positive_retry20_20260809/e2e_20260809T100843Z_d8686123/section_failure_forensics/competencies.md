...
- failed_gate_ids: `x2_competencies_keyword_repetition_limit, x2_no_bullet_outcome_restatement, x2_no_keyword_stuffing, x2_partner_architecture_terms_require_partner_bundle`
...
The section's final materialized artifact did not pass x2_competencies_keyword_repetition_limit, x2_no_bullet_outcome_restatement, x2_no_keyword_stuffing, x2_partner_architecture_terms_require_partner_bundle.
...
| Materialized output | `NOT_OBSERVED` / `NOT_OBSERVED` | `2478caadfa1dab0fcfc2da6df7c61c3d5ae0c02bc5ae8eb28e8a943771e7fe5f` / `C:\Git\apps_rg_v2-worktrees\codex-e2e-defect-remediation-w0\artifacts\apps_rg\runs\w8_anthropic_positive_retry20_20260809\e2e_20260809T100843Z_d8686123\modular_r4\sections\competencies\competencies_display.txt` |
| X2 / X3 | `NOT_OBSERVED` / `NOT_OBSERVED` | `FAIL` / `X3_BLOCK` |
| Judges | `NOT_OBSERVED` | `Google Gemini 3.6 Flash `gemini-3.6-flash`: 5/5 vs 4 PASS; OpenAI ChatGPT `gpt-5.6-sol`: 4.6/5 vs 4 PASS` |

## Required Fix

- Make `competencies` gate final displayed output, not intermediate provider output.
- Bind the failed gate evidence (`x2_competencies_keyword_repetition_limit, x2_no_bullet_outcome_restatement, x2_no_keyword_stuffing, x2_partner_architecture_terms_require_partner_bundle`) to the producer/parser contract that emitted the artifact.
