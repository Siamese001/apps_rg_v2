...
- failed_gate_ids: `x2_headline_executive_abstraction_floor, x2_headline_technical_specificity_floor_met`
...
The section's final materialized artifact did not pass x2_headline_executive_abstraction_floor, x2_headline_technical_specificity_floor_met.
...
| Materialized output | `NOT_OBSERVED` / `NOT_OBSERVED` | `c0fda75ac8bc4fec4e5ac5933bdeeb17f8b4683492105c27358c5edc480a11c9` / `C:\Git\apps_rg_v2-worktrees\codex-e2e-defect-remediation-w0\artifacts\apps_rg\runs\w8_anthropic_positive_retry6_20260809\e2e_20260809T055825Z_cfcbf4fc\modular_r4\sections\headline\headline_output.txt` |
| X2 / X3 | `NOT_OBSERVED` / `NOT_OBSERVED` | `FAIL` / `X3_BLOCK` |
| Judges | `NOT_OBSERVED` | `Google Gemini 3.6 Flash `gemini-3.6-flash`: 5/5 vs 4 PASS; OpenAI ChatGPT `gpt-5.6-sol`: 3.4/5 vs 4 FAIL` |

## Required Fix

- Make `headline` gate final displayed output, not intermediate provider output.
- Bind the failed gate evidence (`x2_headline_executive_abstraction_floor, x2_headline_technical_specificity_floor_met`) to the producer/parser contract that emitted the artifact.
