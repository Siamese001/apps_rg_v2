...
def _run_frozen_validator_scaffold(root: Path, output: Path) -> dict[str, Any]:
    ...
    node = "tests/agentic_core/L2_execution/enforcement/test_manifest_hash_validator.py"
    environment = os.environ.copy()
    environment["PYTHONDONTWRITEBYTECODE"] = "1"
    command = [
        sys.executable,
...
def _evidence_matrix(root: Path, defect_id: str) -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "configuration": _absent_row("agentic_core/L4_state/config/versioned_configs is absent from the frozen tree."),
    ...
    records = [
        {
            ...
            "importing_file": "agentic_core/L2_execution/config/hybrid_retriever_config.py",
            "source_line": 242,
        ...
        },
        {
            ...
            "diagnosis": "INVALID_SOURCE_DEFECT",
            "importing_file": "agentic_core/L2_execution/enforcement/manifest_hash_validator.py",
            "source_line": 108,
            "import_expression": "from agentic_core.L4_state.config.versioned_configs import get_active_configs",
            "requested_module": "agentic_core.L4_state.config.versioned_configs",
            ...
            "complete_local_import_chain": [
                ...
                "agentic_core.L0_routing.enforcement.execution_gateway",
            ...
            "frozen_tests_exercising_intended_behavior": [
                "tests/agentic_core/L2_execution/enforcement/test_manifest_hash_validator.py (import/public-surface scaffold only)",
                "tests/agentic_core/L0_routing/enforcement/test_execution_gateway.py (mocked validator seam)",
            ],
            "frozen_fixtures_exercising_intended_behavior": [],
            "configuration_or_schema_defining_intended_behavior": [],
            "alternative_frozen_implementation": None,
...
def _local_importers(root: Path, package_roots: set[str]) -> dict[str, list[str]]:
    ...
    source_roots = [root / "apps_rg", root / "agentic_core", root / "apps_research"]
...
def third_party_package_reconciliation(root: Path) -> dict[str, Any]:
    ...
    for root_name, rows in sorted(grouped.items()):
        ...
        if root_name == "redis":
            first_importer = "agentic_core/cache/redis_cache_client.py"
            source_chain = [
                "apps_rg.__main__",
...
def _source_change_inventory(root: Path) -> dict[str, Any]:
    ...
    production_prefixes = ("apps_rg/", "agentic_core/", "apps_research/", "apps_eval/")
...
def run_adjudication(root: Path, output_root: Path, run_id: str = DEFAULT_RUN_ID) -> dict[str, Any]:
    ...
    probes = {
        ...
        "W1-IMPORT-002": _resolution_probe(root, "agentic_core.L4_state.config.versioned_configs"),
    }
